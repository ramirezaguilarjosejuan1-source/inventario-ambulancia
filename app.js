...
/* ====== ALMACÉN DE NOCTURNO A ====== */
function inicializarAlmacen() {
  if (!localStorage.getItem("almacen_nocturno_a")) {
    const almacen = {};
    secciones.forEach(sec => {
      sec.items.forEach(i => {
        ...
        if (tipo === "num" || tipo === "bool") {
          if (!almacen[nombre]) {
            almacen[nombre] = {
              stock: (tipo === "num") ? (typeof ideal === 'number' ? ideal * 10 : 10) : 10,
              min: (tipo === "num") ? (typeof ideal === 'number' ? Math.max(ideal * 2, 1) : 1) : 1
            };
          }
        }
      });
    });
    localStorage.setItem("almacen_nocturno_a", JSON.stringify(almacen));
  }
}
inicializarAlmacen();

function obtenerAlmacen() {
  return JSON.parse(localStorage.getItem("almacen_nocturno_a")) || {};
}
...
/* ====== FUNCIONES MODAL ALMACÉN ====== */
function abrirAlmacen() {
  const modal = document.getElementById("modalAlmacen");
  const lista = document.getElementById("listaAlmacen");
  const almacen = obtenerAlmacen();

  lista.innerHTML = "";
  Object.keys(almacen).sort().forEach(nombre => {
    const item = almacen[nombre];
    const esBajo = item.stock <= item.min;
    const div = document.createElement("div");
    div.className = `almacen-item ${esBajo ? 'stock-bajo' : ''}`;
    div.innerHTML = `
      <label>${nombre} ${esBajo ? '⚠️' : ''}</label>
      <div>
        Stock: <input type="number" value="${item.stock}" data-key="stock" data-name="${nombre}">
        Min: <input type="number" value="${item.min}" data-key="min" data-name="${nombre}">
      </div>
    `;
...
function guardarCambiosAlmacen() {
  const almacen = obtenerAlmacen();
  const inputs = document.querySelectorAll("#listaAlmacen input");
  inputs.forEach(input => {
    const name = input.getAttribute("data-name");
    const key = input.getAttribute("data-key");
    if (almacen[name]) {
      almacen[name][key] = Number(input.value);
    }
  });
  guardarAlmacen(almacen);
  alert("✅ Cambios en el almacén guardados");
  cerrarAlmacen();
}

window.onclick = function(event) {
  const modalAlm = document.getElementById("modalAlmacen");
  const modalRes = document.getElementById("modalResurtido");
  if (event.target == modalAlm) cerrarAlmacen();
  if (event.target == modalRes) cerrarResurtido();
}

/* ====== FUNCIONES MODAL RESURTIDO ====== */
function abrirResurtido() {
  const r = JSON.parse(localStorage.getItem("ultimo"));
  if (!r) return;

  const modal = document.getElementById("modalResurtido");
  const lista = document.getElementById("listaResurtido");
  const almacen = obtenerAlmacen();
  ...
  r.secciones.forEach(sec => {
    sec.items.forEach(item => {
      // Si es tipo bool y falta, el faltante es 1 (ideal 1, actual 0)
      let cantFaltante = 0;
      if (item.tipo === "bool") {
          if (item.ideal && !item.actual) cantFaltante = 1;
      } else if (item.tipo === "num") {
          cantFaltante = item.faltante;
      }

      if (cantFaltante > 0) {
        hayFaltantes = true;
        const nombre = item.nombre;
        const stockDisponible = almacen[nombre] ? almacen[nombre].stock : 0;

        const div = document.createElement("div");
        div.className = "almacen-item";
        div.innerHTML = `
          <label>${nombre} (Faltan: ${cantFaltante}, En Almacén: ${stockDisponible})</label>
          <input type="number" value="${Math.min(cantFaltante, stockDisponible)}" min="0" max="${cantFaltante}" data-name="${nombre}">
        `;
        lista.appendChild(div);
      }
    });
  ...
  if (hayFaltantes) {
    ...
    alert("✅ No hay faltantes que resurtir.");
  }
}

function cerrarResurtido() {
  document.getElementById("modalResurtido").style.display = "none";
}

function confirmarResurtido() {
  const almacen = obtenerAlmacen();
  const inputs = document.querySelectorAll("#listaResurtido input");
  let resumen = "✅ Resurtido completado:\n";
  let huboCambio = false;

  inputs.forEach(input => {
    const nombre = input.getAttribute("data-name");
    const cantidad = Number(input.value);
    if (cantidad > 0 && almacen[nombre]) {
      almacen[nombre].stock -= cantidad;
      resumen += `- ${nombre}: ${cantidad} unidades\n`;
      huboCambio = true;
    }
  });
...
function verificarStockBajo() {
  const almacen = obtenerAlmacen();
  const bajos = [];
  Object.keys(almacen).forEach(nombre => {
    if (almacen[nombre].stock <= almacen[nombre].min) {
      bajos.push(nombre);
    }
  });

  if (bajos.length > 0) {
    alert(`⚠️ ALERTA DE DESABASTO ⚠️\nLos siguientes items tienen poco stock en el Almacén de Nocturno A:\n\n${bajos.join("\n")}`);
  }
}
...
async function pdfComparativo() {
  ...
  try {
    ...
    // Iniciar flujo de resurtido
    setTimeout(abrirResurtido, 1000);
  } catch (error) {
    console.error(error);
